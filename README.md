...
しかし、マーケティング部門では、製品紹介資料や客先向けプレゼン資料などから Knowledge Graph を構築出来ると良い。このプロジェクトでは、その手法については掘り下げないが、以下のツールなどを組み合わせて実現出来ること確認済み。しかし、関係抽出が難しい。パワポ資料は単語の箇条書きが多い。スライドのタイトルとテキストボックスの関係で、ある程度の関係抽出が出来る。
