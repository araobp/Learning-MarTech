...
"コトラーのマーケティング5.0"の本では自然言語処理がマーケティングテクノロジーの一つとして説明されている。私は、AI, IoT, XR一通り現場で実践してきたので、私にとって足りないマーケティングテクノロジーのスキルは自然言語処理のみ。
...
国内のマーケティング部門はテクノロジーを軽視するあまり、ITエンジニアやデータサイエンティスト不在な部署が多い。データ利活用を進めているチーム内でもExcelと特定のSaaSしか使われておらず、ER図やSQLという言葉を出したとたん爪弾きにされてしまう。結果として、部署内データはばらばらなExcelファイルに保存されており、その管理に大きな工数がかかっている上に、データの利活用も構造化データの利活用に留まっている（数値で表現できるものへ限定されている）。

しかし、データドリブンなマーケティング実践するには、これらデータを一つのデータベースへ集約するところから始めなければならない。

- 企業の部門内に存在する、ばらばらなExcelファイルをSQLiteへ集約。
- まずはER図でモデリングし、ばらばらなテーブルをつなぐ（＝リレーションの明確化）。
- PythonでETL処理しSQLiteへデータ集約。
- 非構造化データについては、NLPを適用し、見出しをつけることで集約。
- SQLiteからデータを切り出しJSON化しウエブアプリで見せる
- 将来的にはSaaS/PaaSへデータ取り込み
...
SQLiteは埋め込み型のローカルデータベースなので、データの更新も検索もかなり高速。何でもかんでもSaaSやPaaS上のデータベースへデータ格納する前に、SQLiteの利用を検討した方が良い。

=> [SQL](sql)

SQLiteへ集約したデータを可視化する手段としてElectronを検討する。HTML5でネイティブアプリ開発できるので、ブラウザの各種セキュリティー制約を回避出来て好都合。SaaSやPaaSへ依存する前に、SQLite + Electron の組み合わせでプロトタイピングを繰り返した方が良い。そうすれば、現場が本当に必要とするSaaSやPaaSも見えてくる。
...
ローカルPC上でも動作可能な spaCy や Hugging Face 上の2GB以下NLPモデル。Box上に保存されたExcel, PowerpointやPDF資料。ExcelやPowerpoint/PDFをJSON化するツール。これらを組み合わせて使いこなせれば、これまで手がついていなかった非構造化データの利活用に踏み込める。各部署のデータサイエンティストが、部署特化の非構造化データ利活用を行える。ローカルPCとオープンソース、費用は全くかからない。
